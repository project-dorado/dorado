...
| **Real DSP audio analysis** | Phase 13 caveat | `AudioFeatureExtractor` is a documented metadata prior behind the `IAudioAnalysisService` seam; a decode+STFT extractor can replace it without downstream changes (candidate future track). |
